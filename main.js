...
var FTMSchemaServiceClass = class {
   ...
   */
  registerSchema(schema4) {
    const name = schema4.name;
    if (!name)
      return;
    this.customSchemas.set(name, schema4);
  ...
  resolveSchema(schemaName) {
    ...
    let schema4 = ALL_SCHEMAS[schemaName];
    if (!schema4) {
    ...
    let allProperties = {};
    if (schema4.extends && schema4.extends.length > 0) {
      for (const parentName of schema4.extends) {
    ...
    if (schema4.properties) {
      allProperties = { ...allProperties, ...schema4.properties };
    }
    const required = schema4.required || [];
    const featured = schema4.featured || [];
    ...
    const resolved = {
      name: schemaName,
      label: schema4.label || schemaName,
      plural: schema4.plural || schemaName + "s",
      description: schema4.description || "",
      extends: schema4.extends || [],
      abstract: schema4.abstract,
      matchable: schema4.matchable,
      generated: schema4.generated,
      featured,
      required,
      caption: schema4.caption || [],
  ...
  getIntervalSchemas() {
    this.initialize();
    return Array.from(this.resolvedSchemas.values()).filter(
      (schema4) => !schema4.abstract && this.extendsInterval(schema4)
    );
  }
  /**
   * Check if a schema extends from Interval (directly or indirectly).
   */
  extendsInterval(schema4) {
    if (schema4.name === "Interval")
      return false;
    if (schema4.extends.includes("Interval"))
      return true;
    for (const parentName of schema4.extends) {
  ...
  getLabelField(schemaName) {
    const schema4 = this.getSchema(schemaName);
    if (!schema4)
      return "name";
    for (const field of schema4.caption) {
      if (schema4.allProperties[field]) {
        return field;
      }
    }
    return "name";
  ...
  getProperty(schemaName, propertyName) {
    ...
    if (!schema4)
      return null;
    return schema4.allProperties[propertyName] || null;
  }
  /**
...
function getFTMEntityConfig(schemaName) {
  ...
  if (!schema4)
    return null;
  return {
    color: schema4.color || "#607D8B",
    label: schema4.label,
    plural: schema4.plural,
    description: schema4.description,
    labelField: ftmSchemaService.getLabelField(schemaName),
    requiredProperties: schema4.required,
    featuredProperties: schema4.featured,
    optionalProperties: schema4.optionalProperties,
    propertyDefinitions: schema4.allProperties
  };
}
function getAvailableFTMEntityTypes() {
  const types = ftmSchemaService.getEntitySchemas().map((schema4) => ({
    name: schema4.name,
    label: schema4.label,
    description: schema4.description,
    color: schema4.color || "#607D8B"
...
function getAvailableFTMIntervalTypes() {
  const types = ftmSchemaService.getIntervalSchemas().filter((schema4) => !["Associate", "UnknownLink", "Membership"].includes(schema4.name)).map((schema4) => ({
    name: schema4.name,
    label: schema4.label,
    description: schema4.description,
    color: schema4.color || "#607D8B"
...
function getEntityIcon(type) {
  ...
  if (ftmSchemaService.hasSchema(type)) {
    const schema4 = ftmSchemaService.getSchema(type);
  }
  return "\u{1F4E6}";
}
...
var EntityManager = class {
  constructor(app, basePath = "OSINTCopilot", vaultLockService) {
    this.entities = /* @__PURE__ */ new Map();
    this.connections = /* @__PURE__ */ new Map();
    this.vaultLockService = null;
  ...
  assertPathWritable(path, action) {
    if (!path)
      return true;
    if (this.vaultLockService?.isPathLocked(path)) {
      new import_obsidian2.Notice(`Cannot ${action}: note is locked. Unlock from the editor toolbar or plugin settings.`);
      return false;
    }
    return true;
  }
  ...
  async loadFamilyEntityTree(folder, family) {
    for (const typeFolder of folder.children) {
      if (!(typeFolder instanceof import_obsidian2.TFolder))
        continue;
  ...
  async loadLegacyFlatTypeFolder(folder) {
   ...
   */
  async parseEntityFromNote(file, defaultFamily) {
    try {
      ...
      const ftmSchema = frontmatter.ftmSchema;
      let label = frontmatter.label || "";
      if (!label) {
        if (this.schemaCatalog) {
          ...
          label = ftmSchema ? ftmSchemaService.getEntityLabel(ftmSchema, properties) : getEntityLabel(type, properties);
        }
      }
      const entity = {
        id: frontmatter.id,
  ...
  async parseConnectionFromNote(file) {
    try {
      ...
      const ftmSchema = frontmatter.ftmSchema;
      const schemaFamily = frontmatter.schemaFamily ?? (ftmSchema ? "ftm" : "ftm");
      return {
        id: frontmatter.id,
   ...
   */
  async createCatalogEntity(cat, properties, options) {
    if (cat.family === "ftm") {
    ...
    const label = rawLabel !== void 0 && rawLabel !== null && String(rawLabel).trim() !== "" ? String(rawLabel).trim() : `New ${cat.label}`;
    const entity = {
      id,
      type: cat.name,
      label,
  ...
  resolveEntityStorageFolder(entity, typeFolderName) {
    if (entity.filePath) {
      const p = entity.filePath;
      const lastSlash = p.lastIndexOf("/");
      if (lastSlash > 0) {
        return (0, import_obsidian2.normalizePath)(p.substring(0, lastSlash));
      }
    }
    const family = entity.schemaFamily ?? "ftm";
    return (0, import_obsidian2.normalizePath)(`${this.basePath}/${family}/${typeFolderName}`);
  }
  /**
...
`;
  ...
  buildCatalogPropertiesSection(entity, cat) {
    const lines = [];
...
`;
  ...
  async updateEntity(entityId, properties) {
    ...
    if (!entity)
      return null;
    if (!this.assertPathWritable(entity.filePath, "update entity"))
      return null;
  ...
  async restoreEntity(entity) {
    try {
      const family = entity.schemaFamily ?? "ftm";
      const folderPath = (0, import_obsidian2.normalizePath)(`${this.basePath}/${family}/${String(entity.type)}`);
      await this.ensureFolderExists(folderPath);
      const content = this.generateNoteContent(entity);
      const filePath = entity.filePath || `${folderPath}/${sanitizeFilename(entity.label)}.md`;
      if (!this.assertPathWritable(filePath, "restore entity"))
        return false;
      const existingFile = this.app.vault.getAbstractFileByPath(filePath);
      if (existingFile) {
        if (existingFile instanceof import_obsidian2.TFile) {
   ...
   */
  async createConnection(fromEntityId, toEntityId, relationship, properties, options) {
    const fromEntity = this.entities.get(fromEntityId);
    const toEntity = this.entities.get(toEntityId);
    if (!fromEntity || !toEntity)
      return null;
    if (!this.assertPathWritable(fromEntity.filePath, "add connection"))
      return null;
    if (!this.assertPathWritable(toEntity.filePath, "add connection"))
      return null;
    const id = generateId();
    ...
    const connection = {
      ...
      ftmSchema: isFtmFamily ? relationship : void 0,
      schemaFamily: family
    };
  ...
  async updateConnection(connectionId, properties) {
    ...
    if (!fromEntity || !toEntity)
      return null;
    if (!this.assertPathWritable(connection.filePath, "update connection"))
      return null;
...
`;
  ...
  async addRelationshipToNote(fromEntity, toEntity, relationship) {
    if (!fromEntity.filePath)
      return;
    if (!this.assertPathWritable(fromEntity.filePath, "update entity note"))
      return;
  ...
  async addIncomingRelationshipToNote(targetEntity, sourceEntity, relationship) {
    if (!targetEntity.filePath)
      return;
    if (!this.assertPathWritable(targetEntity.filePath, "update entity note"))
      return;
...
`;
  ...
  async restoreConnection(connection) {
    ...
    const toEntity = this.entities.get(connection.toEntityId);
    if (fromEntity && toEntity) {
      if (!this.assertPathWritable(fromEntity.filePath, "restore connection"))
        return false;
      if (!this.assertPathWritable(toEntity.filePath, "restore connection"))
        return false;
      if (connection.filePath && !this.assertPathWritable(connection.filePath, "restore connection"))
        return false;
    }
    this.connections.set(connection.id, connection);
    if (fromEntity && toEntity) {
...
var ClaudeCodeService = class {
  ...
  setVaultSkillResolver(resolver) {
...
Rules: Relationships UPPERCASE. Notes comprehensive. Every Event MUST have start_date (never "unknown") and add_to_timeline:true. Create Location for every place/city/country mentioned. If no entities: {"operations":[]}`;
  ...
  async buildPrompt(text, existingEntities) {
...
function legacyFlagsForChatMode(mode) {
...
function inferChatModeFromLegacyFields(meta) {
  if (meta.orchestrationMode || meta.vaultGraphIngestMode)
    return "general";
...
var ConversationService = class {
  ...
  parseMetadataFromContent(content, filePath) {
    ...
    const localSearchMode = localSearchModeValue === null ? !orchestrationMode && !vaultGraphIngestMode : localSearchModeValue === "true";
    const rawChatMode = this.extractYamlValue(frontmatter, "chatMode");
    const chatModeParsed = rawChatMode === "general" || rawChatMode === "graph" || rawChatMode === "local" ? rawChatMode : null;
    const chatMode = chatModeParsed ?? inferChatModeFromLegacyFields({
      localSearchMode,
      graphGenerationMode,
      orchestrationMode,
    ...
    const taskAgentId = taskAgentIdRaw !== null && taskAgentIdRaw !== "" && taskAgentIdRaw !== '""' ? taskAgentIdRaw.replace(/^["']|["']$/g, "") : "";
    return {
      id,
      title,
      createdAt,
      ...
      messageCount,
      chatMode,
      taskAgentId: taskAgentId || void 0,
      localSearchMode,
      graphGenerationMode,
      orchestrationMode,
      vaultGraphIngestMode
  ...
  async createConversation(firstMessage, chatMode = "general", taskAgentId = "") {
    ...
    const conversation = {
      ...
      messageCount: 0,
      chatMode,
      taskAgentId: taskAgentId.trim() || void 0,
      ...flags,
      messages: []
    };
  ...
  serializeConversation(conversation) {
    ...
    const frontmatterLines = [
      ...
      `chatMode: ${mode}`,
      ...conversation.taskAgentId ? [`taskAgentId: ${conversation.taskAgentId}`] : [],
...
var FTMEntityCreationModal = class extends import_obsidian6.Modal {
  ...
  createCatalogPropertyField(container, propertyName, cat, isRequired) {
    ...
    if (ptype === "text" || propertyName === "description" || propertyName === "notes") {
      ...
      input = fieldContainer.createEl("input", { type: "date" });
    } else if (ptype === "number") {
      input = fieldContainer.createEl("input", { type: "number", placeholder: "\u2026" });
      input.step = "any";
    } else if (ptype === "url") {
      input = fieldContainer.createEl("input", { type: "url", placeholder: "https://\u2026" });
    } else if (ptype === "email") {
      input = fieldContainer.createEl("input", { type: "email" });
    } else {
      input = fieldContainer.createEl("input", {
        type: "text",
        placeholder: `Enter ${(def?.label || propertyName).toLowerCase()}...`
  ...
  async handleCreate() {
    if (this.catalogType && this.catalogType.family !== "ftm") {
      ...
      try {
        ...
        new import_obsidian6.Notice(`Created ${cat.label}: ${entity.label}`);
...
var FTMEntityTypeSelectorModal = class _FTMEntityTypeSelectorModal extends import_obsidian6.Modal {
  ...
  onOpen() {
    ...
    newTypeBtn.onclick = () => {
      this.close();
      const plugin2 = this.app.plugins.plugins["osint-copilot"];
      if (plugin2 && plugin2.customTypesService) {
        new CustomTypeCreationModal(this.app, plugin2.customTypesService, void 0, (name) => {
    ...
    if (plugin?.schemaCatalogService) {
      ...
      famRow.style.cssText = "display: flex; flex-wrap: wrap; gap: 10px 16px; margin-bottom: 12px; align-items: center;";
      famRow.createEl("span", { text: "Show:", cls: "setting-item-description" });
      const ef = plugin.settings.enabledSchemaFamilies;
      const addFamToggle = (key, caption) => {
  ...
  filterEntityTypes() {
    ...
    if (this.useCatalogUi) {
      const filtered2 = this.catalogEntityTypes.filter(
        (cat) => cat.label.toLowerCase().includes(query) || cat.description.toLowerCase().includes(query) || cat.name.toLowerCase().includes(query) || cat.family.toLowerCase().includes(query)
      );
      this.renderCatalogEntityTypes(filtered2);
      return;
    }
  ...
  renderCatalogEntityTypes(types) {
...
var FTMIntervalTypeSelectorModal = class _FTMIntervalTypeSelectorModal extends import_obsidian6.Modal {
  ...
  onOpen() {
    ...
    if (intPlugin?.schemaCatalogService) {
      ...
      famRow.style.cssText = "display: flex; flex-wrap: wrap; gap: 10px 16px; margin-bottom: 12px; align-items: center;";
      famRow.createEl("span", { text: "Show:", cls: "setting-item-description" });
      const ef = intPlugin.settings.enabledSchemaFamilies;
      const addIntToggle = (key, caption) => {
        const wrap = famRow.createDiv();
        wrap.style.display = "flex";
        wrap.style.alignItems = "center";
        wrap.style.gap = "6px";
  ...
  filterIntervalTypes() {
    const query = this.searchInput?.value.toLowerCase() || "";
    if (this.useCatalogUi) {
      const filtered2 = this.catalogIntervalTypes.filter(
        (rel) => rel.label.toLowerCase().includes(query) || rel.description.toLowerCase().includes(query) || rel.name.toLowerCase().includes(query) || rel.family.toLowerCase().includes(query)
      );
      this.renderCatalogIntervalTypes(filtered2);
      return;
    }
  ...
  renderCatalogIntervalTypes(types) {
    if (!this.gridContainer)
      return;
    this.gridContainer.empty();
    if (types.length === 0) {
...
var FTMIntervalCreationModal = class extends import_obsidian6.Modal {
  constructor(app, entityManager, intervalType, onConnectionCreated, preselectedSourceId, preselectedTargetId, catalogRel) {
    ...
    this.targetEntityId = null;
    this.entities = [];
    this.catalogRel = null;
    this.catalogSourceId = null;
    this.catalogTargetId = null;
    this.entityManager = entityManager;
  ...
  createCatalogRelPropertyField(container, propertyName, rel, isRequired) {
    ...
    let input;
    if (ptype === "text") {
      input = fieldContainer.createEl("textarea", { placeholder: "\u2026" });
      input.rows = 3;
    } else if (ptype === "date") {
      input = fieldContainer.createEl("input", { type: "date" });
    } else if (ptype === "number") {
      input = fieldContainer.createEl("input", { type: "number" });
      input.step = "any";
    } else {
      input = fieldContainer.createEl("input", { type: "text" });
    }
    input.addClass("graph_copilot-entity-input");
    input.addEventListener("input", () => {
      if (input.type === "number") {
...
var _GraphView = class _GraphView extends import_obsidian10.ItemView {
  constructor(leaf, entityManager, onEntityClick, onShowOnMap, graphHost) {
    ...
    this.allGraphPositions = {};
    this.graphSelectEl = null;
  ...
  createToolbar(toolbar) {
    ...
    this.lockAreaBtn = toolbar.createEl("button", { text: "\u{1F512} lock area" });
   ...
   */
  async switchGraphWorkspace(newId) {
  ...
  initializeGraph() {
    ...
    this.cy.on("mouseover", "node, edge", (evt) => {
      ...
      const isNode2 = ele.isNode();
      this.showGraphTooltip(ele, isNode2, evt.renderedPosition);
    });
   ...
   */
  showGraphTooltip(ele, isNode2, renderPos) {
    this.hideGraphTooltip();
    ...
    try {
      ...
      if (file instanceof import_obsidian10.TFile) {
        ...
        if (raw && raw.version === 2 && raw.byGraph && typeof raw.byGraph === "object") {
          this.allGraphPositions = raw.byGraph;
        } else {
          const legacy = raw;
...
var CustomTypesService = class {
    ...
    if (await this.vault.adapter.exists(configPath)) {
      try {
        ...
        if (config.schemas && Array.isArray(config.schemas)) {
          for (const schema4 of config.schemas) {
            if (schema4.name) {
  ...
  async updateCustomType(schema4) {
    if (!schema4.name || !this.customSchemas.has(schema4.name)) {
      throw new Error(`Custom type ${schema4.name} does not exist`);
    }
    await this.addCustomType(schema4);
  }
...
*The planner request failed; adjust modules and click Run.*`,
  ...
  mimeTypeForIngestExtension(ext) {
    ...
    return map2[e] || "application/octet-stream";
  }
  ...
  async runVaultGraphIngest(onFileProgress, abortSignal) {
    ...
    const refTime = (/* @__PURE__ */ new Date()).toISOString();
...
${text}`);
      ...
      try {
        ...
        extraction = await this.plugin.graphApiService.processTextInChunks(
          ...
          (chunkIndex, totalChunks, message) => {
            if (abortSignal?.aborted)
              return;
            const scaled = basePct + 2 + Math.floor(chunkIndex / Math.max(totalChunks, 1) * (80 / totalBatches));
    ...
    const summary = (abortSignal?.aborted ? "**Cancelled by user.** " : "") + `Processed **${filesProcessed}** file(s) out of **${files.length}** eligible (cap ${_OrchestrationService.VAULT_INGEST_MAX_FILES}), in **${totalBatches}** batch(es) of up to ${BATCH} files (**local Claude** extraction). ` + (extractFailures > 0 ? `**${extractFailures}** file(s) or batch(es) had issues. ` : "") + `**${graphCommands.length}** graph operation(s) were **applied automatically** to your vault graph.`;
    return {
      summary,
      graphCommands,
      filesProcessed,
...
Content Preview: ${content.substring(0, 500)}...`);
          ...
          case "EXTRACT_TO_GRAPH": {
            ...
            const extractSignal = options?.abortSignals?.["EXTRACT_TO_GRAPH"] ?? options?.globalAbort;
            ...
            const onRetry = (attempt, maxAttempts, _reason, _nextDelayMs) => {
              onProgress(
                displayName,
            ...
            results["EXTRACT_TO_GRAPH"] = {
              ...
              graphCommands,
              summary: extraction.success ? graphCommands.length > 0 ? `Claude extracted **${graphCommands.length}** graph command(s) from attachment text. Confirm with **\u{1F4CA} Generate Analysis & Graph**.` : "No entities or relationships were extracted from the attachment text." : `Extraction failed: ${extraction.error || "unknown error"}`
            };
            onProgress(displayName, "Complete", 100);
            break;
          }
...
`;
      ...
      if (tool === "EXTRACT_TO_GRAPH" && result && typeof result === "object" && result.__extractToGraph === true && Array.isArray(result.graphCommands)) {
        const r = result;
      ...
      try {
        if (command.startsWith("@@create_entity")) {
          ...
          if (data.id) {
            ...
            const locked = conn?.filePath && this.plugin.vaultLockService?.isPathLocked(conn.filePath) || false;
            if (locked) {
...
function parseMarkdownWithFrontmatter(raw) {
  ...
  if (end === -1) {
    ...
    if (end !== -1) {
      const fmBlock2 = trimmed.slice(3, end).trim();
      const body2 = trimmed.slice(end + 5).trim();
      return { data: parseYamlLike(fmBlock2), body: body2 };
    }
    return { data: {}, body: trimmed };
  }
...
var VaultPromptLoader = class {
  constructor(app, getPromptsRoot, getActiveAgentId) {
    ...
    this.getActiveAgentId = getActiveAgentId;
  ...
  async getOrchestrationAugmentation() {
    ...
    const parts = [];
    if (global)
      parts.push("=== USER VAULT RULES (rules/global.md) ===\n" + global);
    if (agent)
      parts.push(
  ...
  async listAgents() {
    ...
    for (const child of folder.children) {
      ...
      try {
        ...
        const id = data.id || child.basename.replace(/\.md$/i, "") || child.basename;
        out.push({
          id,
          name: data.name || id,
...
// src/services/vault-prompt-bootstrap.ts
var VaultPromptBootstrapService = class {
  constructor(app, getPromptsRoot) {
    this.app = app;
    this.getPromptsRoot = getPromptsRoot;
  }
...
// src/task-agents/parse-manifest.ts
function splitList(raw) {
  if (!raw || !raw.trim())
    return [];
  return raw.split(/[,\n]/).map((s) => s.trim()).filter(Boolean).map((p) => normalizeVaultPath(p.replace(/^["']|["']$/g, "")));
}
...
function parseIntSafe(raw, defaultVal) {
  if (raw === void 0 || raw === "")
    return defaultVal;
  const n = parseInt(raw.trim(), 10);
  return Number.isFinite(n) && n > 0 ? n : defaultVal;
}
function parseTaskAgentMarkdown(raw, sourcePath) {
  const { data, body } = parseMarkdownWithFrontmatter(raw);
  ...
  if (!id)
    return null;
  const outputSchema = (data.output_schema || "").trim();
  if (outputSchema !== "vault_files_v1")
    return null;
  ...
  return {
    ...
    maxContextChars: parseIntSafe(data.max_context_chars, 12e4),
...
async function assembleTaskAgentContext(app, contextRoots, pluginIndex, maxNotes, maxChars, query) {
  ...
  for (const [, note] of pluginIndex) {
    ...
    score += Math.min(note.content.length / 5e3, 1);
...
// src/task-agents/path-allowlist.ts
...
function isPathAllowedForWrite(vaultRelativePath, agentOutputRoots, globalAllowlistRoots) {
  ...
  const agentNorm = agentOutputRoots.map((r) => ensureTrailingSlash(r)).filter(Boolean);
  const globalNorm = globalAllowlistRoots.map((r) => ensureTrailingSlash(r)).filter(Boolean);
  if (agentNorm.length === 0 || globalNorm.length === 0)
    return false;
...
function normalizeRootList(raw) {
  return raw.split(/[,\n]/).map((s) => s.trim()).filter(Boolean).map((p) => normalizeVaultPath(p.replace(/^["']|["']$/g, "")));
}

// src/task-agents/json-response.ts
function tryParseVaultFiles(obj) {
  if (!obj || typeof obj !== "object")
    return null;
  const o = obj;
  if (o.version !== "vault_files_v1")
    return null;
  ...
  for (const f of o.files) {
    ...
    files.push({
      ...
      frontmatter: typeof e.frontmatter === "string" ? e.frontmatter : void 0
    });
  }
  return { version: "vault_files_v1", files };
}
function parseVaultFilesJson(raw) {
  const trimmed = raw.trim();
  try {
    const data = JSON.parse(trimmed);
...
async function applyVaultFilesV1(app, data, agentOutputRoots, globalAllowlistRoots, isPathLocked) {
...
function formatApplyNotice(res) {
  const parts = [];
  if (res.created.length)
    parts.push(`Created: ${res.created.length}`);
  if (res.updated.length)
    parts.push(`Updated: ${res.updated.length}`);
...
// src/task-agents/task-agent-runner.ts
var TaskAgentRunner = class {
  constructor(app, getClaude, vaultPromptLoader, getPluginIndex, getDefaultModel, options) {
    this.app = app;
    this.getClaude = getClaude;
    this.vaultPromptLoader = vaultPromptLoader;
    this.getPluginIndex = getPluginIndex;
  ...
  async run(manifest, userMessage, signal) {
    ...
    if (globalRoots.length === 0) {
      return {
        assistantText: "Task agents are disabled: add at least one **global output allowlist** folder in Settings (Task agents).",
        appliedPaths: []
      };
    }
...
Paths must be vault-relative, use forward slashes, and stay within the agent output_roots AND the user's global allowlist.`;
    const parts = [];
    if (globalRules)
      parts.push("=== GLOBAL RULES (vault) ===\n" + globalRules);
    parts.push("=== TASK AGENT INSTRUCTIONS ===\n" + manifest.body);
    if (wiki)
      parts.push("=== WIKI / CONTEXT ===\n" + wiki);
    ...
    if (!data || data.files.length === 0) {
      return {
        assistantText: `Could not parse vault_files_v1 JSON from the model.

--- raw (truncated) ---
${raw.slice(0, 4e3)}${raw.length > 4e3 ? "\n\u2026" : ""}`,
        appliedPaths: []
      };
    }
...
// src/task-agents/task-agent-bootstrap.ts
var TaskAgentBootstrapService = class {
  constructor(app, getTaskAgentsRoot) {
    this.app = app;
...
// src/task-agents/task-agent-settings.ts
function isTaskAgentRunnable(manifest, settings) {
  if (!settings.taskAgentsEnabled)
    return false;
...
var SchemaBootstrapService = class {
  ...
  async ensureDefaultsInstalled() {
    const root = (0, import_obsidian25.normalizePath)(this.getEntityBasePath().trim() || OSINT_COPILOT_VAULT_ROOT);
...
function parseEntityYaml(content, _path) {
...
function parseRelationshipYaml(content, _path) {
  const doc = parse(content);
  if (!doc || typeof doc !== "object") {
    throw new Error("Invalid YAML: expected object root");
  }
...
function parseCombinedSchemaYaml(content) {
  const doc = parse(content);
  if (!doc?.family)
    throw new Error("Invalid YAML: missing family");
...
function yamlEntityToCatalog(input, family) {
...
function yamlRelationshipToCatalog(input, family) {
  const props = normalizeProperties(input.properties);
  return {
    family,
    name: input.name,
...
// src/services/schema-catalog-types.ts
var DEFAULT_ENABLED_SCHEMA_FAMILIES = {
  ftm: true,
  stix2: true,
  mitre: true,
  user: true
...
// src/services/schema-catalog-service.ts
function ftmPropsToCatalog(all) {
  const out = {};
...
var SchemaCatalogService = class {
  constructor(app, getEntityBasePath) {
    this.entities = /* @__PURE__ */ new Map();
  ...
  async loadVaultStix2() {
    const base = (0, import_obsidian26.normalizePath)(`${this.getEntityBasePath()}/schemas/stix2`);
  ...
  async loadVaultMitre() {
    const base = (0, import_obsidian26.normalizePath)(`${this.getEntityBasePath()}/schemas/mitre`);
    await this.loadEntityYamlFile(`${base}/entities.yaml`, "mitre");
    await this.loadRelationshipYamlFile(`${base}/relationships.yaml`, "mitre");
  }
  async loadEntityYamlFile(path, expectedFamily) {
    try {
  ...
  async loadRelationshipYamlFile(path, expectedFamily) {
    try {
      if (!await this.app.vault.adapter.exists(path))
        return;
      const content = await this.app.vault.adapter.read(path);
  ...
  async loadVaultUser() {
    const dir = (0, import_obsidian26.normalizePath)(`${this.getEntityBasePath()}/schemas/user`);
    const folder = this.app.vault.getAbstractFileByPath(dir);
    if (!folder || !(folder instanceof import_obsidian26.TFolder))
      return;
    for (const child of folder.children) {
      if (!(child instanceof import_obsidian26.TFile))
        continue;
      const lower = child.name.toLowerCase();
      if (!lower.endsWith(".yaml") && !lower.endsWith(".yml"))
        continue;
      if (child.name === "example-user-types.yaml")
        continue;
      try {
  ...
  listEntityTypes(enabled) {
    const list = [];
    for (const [, v] of this.entities) {
      if (!this.isFamilyEnabled(v.family, enabled))
        continue;
      list.push(v);
    }
  ...
  isFamilyEnabled(family, enabled) {
    switch (family) {
      case "ftm":
        return enabled.ftm;
      case "stix2":
...
function mergeEnabledFamilies(partial) {
...
var VaultAIPlugin = class extends import_obsidian27.Plugin {
  constructor() {
    super(...arguments);
    this.index = /* @__PURE__ */ new Map();
    this.claudeCodeService = null;
  }
  attachVaultSkillFromVault() {
  ...
  getGraphEntityVisual(entity) {
    ...
    return { color: getEntityColor(entity.type), icon };
  }
  async onload() {
    ...
    let schemaCatalogDebounce = null;
    const scheduleSchemaCatalogRebuild = () => {
      if (schemaCatalogDebounce !== null)
        window.clearTimeout(schemaCatalogDebounce);
      schemaCatalogDebounce = window.setTimeout(() => {
        void this.schemaCatalogService?.rebuild();
      }, 650);
    };
    const isUnderEntitySchemas = (path) => {
      const base = (0, import_obsidian27.normalizePath)(this.settings.entityBasePath.trim() || "OSINTCopilot");
      const prefix = (0, import_obsidian27.normalizePath)(`${base}/schemas`);
      const p = (0, import_obsidian27.normalizePath)(path);
      return p === prefix || p.startsWith(`${prefix}/`);
    };
    this.registerEvent(
      this.app.vault.on("modify", (file) => {
        if (file instanceof import_obsidian27.TFile && isUnderEntitySchemas(file.path)) {
    ...
    this.addCommand({
      ...
      callback: () => {
        try {
          ...
          a.click();
          URL.revokeObjectURL(url);
          new import_obsidian27.Notice("Investigation exported successfully");
        } catch (error) {
          console.error("[OSINT Copilot] Export failed:", error);
          new import_obsidian27.Notice("Failed to export investigation");
        }
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      callback: () => {
        void this.buildIndex().then(() => {
          new import_obsidian27.Notice("Vault reindexed successfully.");
        });
      }
    });
    this.addCommand({
      ...
      callback: () => {
        void this.entityManager.loadEntitiesFromNotes().then(() => {
          new import_obsidian27.Notice("Entities reloaded from notes.");
        });
      }
    });
    this.addCommand({
      ...
      callback: () => {
        ...
        this.attachVaultSkillFromVault();
        new import_obsidian27.Notice("Vault prompts, skills, and task-agent registry refreshed.");
      }
    });
    this.addCommand({
      ...
      callback: () => {
        void (async () => {
          try {
            ...
            this.skillRegistry?.invalidate();
            this.attachVaultSkillFromVault();
            new import_obsidian27.Notice("Missing default prompt, skills, and task-agent files were created (existing files unchanged).");
          } catch (e) {
            new import_obsidian27.Notice(`Failed: ${e instanceof Error ? e.message : String(e)}`, 5e3);
          }
        })();
      }
    });
  ...
  async setActiveGraphId(id) {
    if (!this.settings.graphWorkspaces.some((g) => g.id === id))
      return;
  ...
  registerVaultLockEditorHooks() {
    const update = (leaf) => this.updateVaultLockLeafMode(leaf);
  ...
  updateVaultLockLeafMode(leaf) {
    ...
    if (locked) {
      ...
      if (state.state && "mode" in state.state && state.state.mode === "source") {
  ...
  addVaultUnlockButton(view, path) {
    this.removeVaultUnlockButton(view);
    const container = view.containerEl;
    ...
    (0, import_obsidian27.setIcon)(btn, "lock");
    btn.addEventListener("click", () => {
      new VaultUnlockModal(this.app, () => {
        this.vaultLockService.unlockPath(path);
        this.app.workspace.iterateAllLeaves((leaf) => {
          if (leaf.view instanceof import_obsidian27.MarkdownView && leaf.view.file?.path === path) {
  ...
  async openGraphView(forceNew = false) {
    if (!this.settings.enableGraphFeatures) {
      new import_obsidian27.Notice("Graph features are disabled. Enable them in settings \u2192 osint copilot \u2192 enable graph features", 5e3);
  ...
  async refreshGraphView() {
    ...
    if (existing.length > 0) {
      ...
      if (graphView && typeof graphView.refreshWithSavedPositions === "function") {
        ...
        new import_obsidian27.Notice("Graph view updated with new entities");
      }
    }
  }
  /**
  ...
  async refreshOrOpenGraphView() {
    ...
    if (existing.length > 0) {
      ...
      if (this.settings.autoOpenGraphOnEntityCreation) {
        ...
        new import_obsidian27.Notice("Graph view opened with new entities");
      }
    }
  }
  /**
  ...
  async showEntityOnMap(entityId) {
    const entity = this.entityManager.getEntity(entityId);
    if (!entity) {
      new import_obsidian27.Notice("Entity not found");
      return;
    }
    if (entity.type !== "Location") {
      new import_obsidian27.Notice("Only location entities can be shown on the map");
      return;
    }
    if (!entity.properties.latitude || !entity.properties.longitude) {
      new import_obsidian27.Notice("Location has no coordinates. Please add latitude and longitude.");
      return;
    }
...
var AskModal = class extends import_obsidian27.Modal {
  ...
  async handleAsk() {
    ...
    if (!query) {
      new import_obsidian27.Notice("Please enter a question.");
      return;
    }
    ...
    try {
      ...
      copyButton.addEventListener("click", () => {
        ...
        new import_obsidian27.Notice("Answer copied to clipboard.");
      });
...
var RenameConversationModal = class extends import_obsidian27.Modal {
...
var _ChatView = class _ChatView extends import_obsidian27.ItemView {
  ...
  async openSkillsMenu(anchor) {
    ...
    try {
      ...
      new import_obsidian27.Notice("Could not load skills.", 4e3);
    }
    menu.addSeparator();
    menu.addItem((item) => {
  ...
  async createNewSkillFile() {
    const root = (0, import_obsidian27.normalizePath)(this.plugin.settings.skillsFolder.trim() || DEFAULT_SKILLS_FOLDER);
    const id = `new_skill_${Date.now()}`;
    const fileName = `new-skill-${Date.now()}.md`;
    const path = (0, import_obsidian27.normalizePath)(`${root}/${fileName}`);
    const body = `---
skill_kind: vault
id: ${id}
name: New skill
...
Instructions for this skill (local Claude).`;
    try {
      ...
      const f = this.app.vault.getAbstractFileByPath(path);
      if (f instanceof import_obsidian27.TFile) {
        await this.app.workspace.getLeaf(false).openFile(f);
      }
      this.plugin.skillRegistry.invalidate();
      new import_obsidian27.Notice(`Created ${path}`);
    } catch (e) {
      new import_obsidian27.Notice(`Failed to create skill: ${e instanceof Error ? e.message : String(e)}`, 5e3);
    }
  }
  ...
  async loadMostRecentConversation() {
    ...
    if (conversation) {
      ...
      this.selectedTaskAgentId = conversation.taskAgentId ?? this.plugin.settings.preferredTaskAgentId ?? "";
    } else {
      this.applyChatMode("general");
  ...
  async render() {
    ...
    this.agentDropdown.addEventListener("change", () => {
      ...
      new import_obsidian27.Notice(`Active agent: ${this.plugin.settings.activeAgentId}`);
    });
    ...
    inputContainer.addEventListener("drop", async (e) => {
      ...
      if (dragManager?.draggable) {
        ...
        if (draggable.file instanceof import_obsidian27.TFolder) {
          await this.handleDroppedFolder(draggable.file);
          handled = true;
        } else if (draggable.file instanceof import_obsidian27.TFile) {
        ...
        if (!handled && Array.isArray(draggable.files) && draggable.files.length > 0) {
          for (const f of draggable.files) {
            if (f instanceof import_obsidian27.TFolder)
              await this.handleDroppedFolder(f);
            else if (f instanceof import_obsidian27.TFile)
        ...
        if (!handled && draggable.info) {
          ...
          if (typeof info === "string") {
            ...
            if (resolved instanceof import_obsidian27.TFolder) {
              await this.handleDroppedFolder(resolved);
              handled = true;
            } else if (resolved instanceof import_obsidian27.TFile) {
      ...
      if (!handled && e.dataTransfer) {
        ...
        if (data) {
          ...
          if (abstractFile instanceof import_obsidian27.TFolder) {
            await this.handleDroppedFolder(abstractFile);
            handled = true;
          } else if (abstractFile instanceof import_obsidian27.TFile) {
  ...
  showUrlInputModal() {
    ...
    extractBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        await this.renderMessages();
        new import_obsidian27.Notice(`Extracted content from URL. Processing entities...`);
  ...
  async handleFileUpload(event) {
    ...
    if (!_ChatView.ALLOWED_EXTENSIONS.has(ext)) {
      new import_obsidian27.Notice(`File type .${ext} not supported. Use images or documents.`);
      return;
    }
    this.attachedFiles.push({ file, extracted: false });
    this.renderAttachments();
    new import_obsidian27.Notice(`Attached: ${file.name}`);
  }
  ...
  async handleDroppedFile(file) {
    ...
    const ext = (file.name.split(".").pop() || "").toLowerCase();
    if (!_ChatView.ALLOWED_EXTENSIONS.has(ext)) {
      new import_obsidian27.Notice(`File type .${ext} not supported. Use images (.jpg, .png, etc.) or documents (.pdf, .docx, .txt)`);
      return;
    }
    this.attachedFiles.push({ file, extracted: false });
    this.renderAttachments();
  ...
  async handleDroppedFolder(folder) {
    ...
    if (files.length === 0) {
      new import_obsidian27.Notice(`No supported files found in folder "${folder.name}"`);
      return;
    }
    for (const file of files) {
      this.attachedFiles.push({ file, extracted: false });
    ...
    this.renderAttachments();
    new import_obsidian27.Notice(`Attached ${files.length} file${files.length > 1 ? "s" : ""} from folder "${folder.name}"`);
  }
  collectFilesFromFolder(folder) {
    const results = [];
    for (const child of folder.children) {
      if (child instanceof import_obsidian27.TFile) {
  ...
  async handleUrlExtraction(url) {
    try {
      ...
      new import_obsidian27.Notice(`Extracting text from URL: ${url}...`);
      const text = await this.plugin.graphApiService.extractTextFromUrl(url);
      this.inputEl.value = text;
      new import_obsidian27.Notice(`Text extracted from URL`);
      return true;
    } catch (error) {
      console.error("URL extraction error:", error);
      new import_obsidian27.Notice(`Error extracting URL: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    } finally {
  ...
  checkGraphOnlyMode() {
    if (this.isGraphOnlyMode()) {
      new import_obsidian27.Notice("Graph only mode - enter text to extract entities");
    }
  }
  ...
  async loadConversation(id) {
    ...
    if (conversation) {
      ...
      new import_obsidian27.Notice("Failed to load conversation");
    }
  }
  async startNewConversation() {
    ...
    await this.render();
    new import_obsidian27.Notice("Started new conversation");
  }
  async deleteConversation(id) {
    new ConfirmModal(
      this.app,
      ...
      async () => {
        ...
        await this.renderMessages();
        if (success) {
          new import_obsidian27.Notice("Conversation deleted");
        } else {
          new import_obsidian27.Notice("Failed to delete conversation");
        }
      },
      void 0,
      true
    ...
    new RenameConversationModal(this.app, currentTitle, (newTitle) => {
      void (async () => {
        ...
        if (success) {
          ...
          this.renderConversationList();
          new import_obsidian27.Notice("Conversation renamed");
        }
      })();
    }).open();
  }
            ...
            if (fullEntity.filePath) {
              void (async () => {
                ...
                if (file instanceof import_obsidian27.TFile) {
                  ...
                  new import_obsidian27.Notice("Linked note file not found");
                }
              })();
            }
          });
      ...
      if (item.role === "assistant" && item.reportFilePath) {
        ...
        reportButton.addEventListener("click", (e) => {
          ...
          void (async () => {
            ...
            if (file instanceof import_obsidian27.TFile) {
              await this.app.workspace.getLeaf().openFile(file);
              new import_obsidian27.Notice(`Opened report: ${item.reportFilePath}`);
            } else {
              new import_obsidian27.Notice(`Companies&People file not found: ${item.reportFilePath}`);
            }
          })();
        });
  ...
  updateProgressBar(messageIndex, progress, intermediateResults) {
    ...
    if (currentProgress) {
      ...
      if (vaultLive && vaultLive.length > 0) {
        ...
        void import_obsidian27.MarkdownRenderer.render(this.app, vaultLive.join("\n\n"), mdBox, "", this);
  ...
  async handleOrchestrationToolCancel(messageIndex, toolId) {
    ...
    new ConfirmModal(
      ...
      () => {
        ...
        new import_obsidian27.Notice("Task cancelled");
      },
      () => {
      },
      true
  ...
  async handleCancel(index) {
    ...
    new ConfirmModal(
      ...
      () => {
        ...
        new import_obsidian27.Notice("Operation cancelled");
      },
      () => {
      },
      true
    ...
    if (this.attachedFiles.length > 0) {
      ...
      for (const attachment of filesToProcess) {
        ...
        try {
          ...
          } else if (attachment.file instanceof import_obsidian27.TFile) {
...
${text}`);
          ...
          new import_obsidian27.Notice(userMessage, 8e3);
        }
      }
...
${fileList}` : fileList;
      ...
      if (extractedCount > 0 && failedCount === 0) {
        new import_obsidian27.Notice(`Processed ${extractedCount} file${extractedCount > 1 ? "s" : ""}`);
      } else if (extractedCount > 0 && failedCount > 0) {
        new import_obsidian27.Notice(`Processed ${extractedCount} file${extractedCount > 1 ? "s" : ""}, ${failedCount} failed`);
      }
    }
  ...
  async handleVaultTaskAgent(query) {
    ...
    try {
      ...
      if (!isTaskAgentRunnable(manifest, this.plugin.settings)) {
        this.chatHistory[assistantIndex].content = `Task agent **${manifest.name}** is disabled. Enable it under Settings \u2192 Task agents.`;
        this.chatHistory[assistantIndex].progress = void 0;
        this.activeAbortControllers.delete(assistantIndex);
        await this.renderMessages();
        return;
...
${r.snippet || r.content || ""}` : JSON.stringify(r, null, 2);
      ...
      import_obsidian27.MarkdownRenderer.render(this.app, displayText, content, "", this);
      if (tool === "VAULT_GRAPH_INGEST" && item.vaultIngestLiveLog && item.vaultIngestLiveLog.length > 0) {
        ...
        void import_obsidian27.MarkdownRenderer.render(this.app, item.vaultIngestLiveLog.join("\n\n"), logDiv, "", this);
      }
  ...
  renderProposedPlan(item, index, messageDiv) {
    ...
    if (plan.planSummary) {
      ...
      import_obsidian27.MarkdownRenderer.render(this.app, plan.planSummary, summaryDiv, "", this);
    }
    const allTools = [
    ...
    executeBtn.addEventListener("click", () => {
      ...
      if (selectedTools.length === 0) {
        new import_obsidian27.Notice("Please select at least one investigation module.");
        return;
      }
...
**Error:** ${errorMsg}`;
      ...
      new import_obsidian27.Notice(`Graph generation failed: ${errorMsg}`);
    }
  }
...
`;
      ...
      const onDelta = (delta) => {
        ...
        if (contentEl) {
          import_obsidian27.MarkdownRenderer.renderMarkdown(streamed, contentEl, "", this.plugin);
...
var VaultAISettingTab = class extends import_obsidian27.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    /**
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian27.Setting(containerEl).setName("Plugin updates").setHeading();
    new import_obsidian27.Setting(containerEl).setName("Current version: " + this.plugin.manifest.version).setDesc("Force update plugin to the latest version from GitHub main branch. This will overwrite local files with the newest code.").addButton(
      (btn) => btn.setButtonText("Update plugin").setCta().setTooltip("Download and install the latest version from GitHub main branch").onClick(async () => {
        ...
        new import_obsidian27.Notice("Updating plugin from GitHub main branch...");
        try {
          const success = await this.plugin.updaterService.updateFromMain();
          if (success) {
            btn.setButtonText("Reloading...");
            new import_obsidian27.Notice("Update successful! Reloading plugin...");
            ...
            new import_obsidian27.Notice("Failed to download update. Check console for details.");
          }
        } catch (error) {
          ...
          btn.setDisabled(false);
          setTimeout(() => btn.setButtonText(originalText), 3e3);
          new import_obsidian27.Notice("An error occurred during update.");
        }
      })
    );
    new import_obsidian27.Setting(containerEl).setName("Graph note lock").setHeading();
    new import_obsidian27.Setting(containerEl).setName(`Locked notes (${this.plugin.vaultLockService.getLockedCount()})`).setDesc(
      ...
      (btn) => btn.setButtonText("Unlock all").onClick(async () => {
        ...
        new import_obsidian27.Notice("All graph locks cleared.");
        this.display();
      })
    );
    new import_obsidian27.Setting(containerEl).setName("Max notes").setDesc("Maximum number of notes to include in context").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("System prompt").setDesc("Default system prompt for q&a").addTextArea((text) => {
    ...
    new import_obsidian27.Setting(containerEl).setName("Vault prompts").setHeading();
    new import_obsidian27.Setting(containerEl).setName("Prompts folder").setDesc("Editable rules, agents, and graph-extraction skill (Markdown). Default copies on first run if files are missing.").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Active agent id").setDesc("Matches agents/<id>.md under the prompts folder (see frontmatter id).").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Install missing default prompt files").setDesc("Creates any default files that are not present. Does not overwrite your edits.").addButton(
      (btn) => btn.setButtonText("Install missing").onClick(async () => {
        try {
          ...
          this.plugin.skillRegistry?.invalidate();
          this.plugin.attachVaultSkillFromVault();
          new import_obsidian27.Notice("Done. Open the prompts, skills, and task-agents folders in the vault to edit.");
        } catch (e) {
          new import_obsidian27.Notice(`Failed: ${e instanceof Error ? e.message : String(e)}`, 5e3);
        }
      })
    );
    new import_obsidian27.Setting(containerEl).setName("Skills (orchestration)").setHeading();
    new import_obsidian27.Setting(containerEl).setName("Skills folder").setDesc(
    ...
    new import_obsidian27.Setting(containerEl).setName("Task agents (vault)").setHeading();
    new import_obsidian27.Setting(containerEl).setName("Enable task agents").setDesc("In General mode, pick a task agent to run local Claude workflows that create vault files (JSON contract), or None for full orchestration.").addToggle(
      (t) => t.setValue(this.plugin.settings.taskAgentsEnabled).onChange(async (v) => {
        this.plugin.settings.taskAgentsEnabled = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian27.Setting(containerEl).setName("Task agents folder").setDesc("Markdown manifests with agent_kind: task (separate from prompts/agents orchestration agents).").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Global output allowlist").setDesc("Newlines or commas. Task agents may only write under these paths AND each agent's output_roots.").addTextArea((text) => {
    ...
    new import_obsidian27.Setting(containerEl).setName("Install missing default task-agent files").setDesc("Creates README + sample agents when missing. Does not overwrite edits.").addButton(
      (btn) => btn.setButtonText("Install missing").onClick(async () => {
        try {
          await new TaskAgentBootstrapService(this.plugin.app, () => this.plugin.settings.taskAgentsFolder).ensureDefaultsInstalled();
          this.plugin.taskAgentRegistry?.invalidate();
          new import_obsidian27.Notice("Task-agent defaults installed where missing.");
          this.display();
        } catch (e) {
          new import_obsidian27.Notice(`Failed: ${e instanceof Error ? e.message : String(e)}`, 5e3);
        }
    ...
    new import_obsidian27.Setting(containerEl).setName("Conversation history folder").setDesc("Directory where chat conversations will be saved").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Graph extraction (Claude Code)").setHeading();
    ...
    new import_obsidian27.Setting(containerEl).setName("Claude CLI path").setDesc("Path to the claude executable. Use 'claude' if it's on your PATH.").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Claude model").setDesc("Model to use for extraction (e.g. sonnet, opus, haiku).").addText(
    ...
    new import_obsidian27.Setting(containerEl).setName("Test Claude Code").setDesc("Verify that the Claude CLI is reachable.").addButton(
      (btn) => btn.setButtonText("Test connection").onClick(async () => {
        ...
        try {
          ...
          new import_obsidian27.Notice(ok ? "Claude Code CLI is available!" : "Claude CLI not found. Check the path.");
        } catch (e) {
          new import_obsidian27.Notice("Error: " + (e.message || String(e)));
        }
        btn.setButtonText("Test connection");
        btn.setDisabled(false);
      })
    );
    new import_obsidian27.Setting(containerEl).setName("Graph view").setHeading();
    new import_obsidian27.Setting(containerEl).setName("Schema families in type pickers").setDesc(
    ...
    const addFamToggle = (key, name) => {
      new import_obsidian27.Setting(containerEl).setName(name).addToggle(
    ...
    new import_obsidian27.Setting(containerEl).setName("Auto-refresh graph view").setDesc("Automatically refresh the graph view when new entities are created through AI generation").addToggle(
    ...
    new import_obsidian27.Setting(containerEl).setName("Auto-open graph view").setDesc("Automatically open the graph view when entities are created (if not already open)").addToggle(
  ...
  async populateTaskAgentToggleSettings(host) {
    host.empty();
    try {
      ...
      for (const a of list) {
        ...
        new import_obsidian27.Setting(host).setName(a.name).setDesc(`${a.description || a.id} \u2014 output: ${a.outputRoots.join(", ")}`).addToggle((t) => {
          t.setValue(runnable);
          t.onChange(async (v) => {
            if (v) {
              if (a.enabledDefault) {
